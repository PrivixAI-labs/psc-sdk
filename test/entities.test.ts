...
import { ChainId, WPSC as _WETH, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const WPSC = _WETH[ChainId.TESTNET]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WPSC, decimalize(1234, WPSC.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WPSC]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WPSC)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WPSC, decimalize(10, WPSC.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WPSC, '1663192997082117548')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WPSC, '1663192997082117548')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WPSC, '1234567000000000000000')
